...
- Please download the following datasets that will be used in this presentation:
    ...
    - `NYParkingViolations_small.csv`: http://tinyurl.com/df15-parking
...
park = read_csv("NYParkingViolations_small.csv")
park
```
...
```{r}
group_by(select(park, 1:6), `Registration State`)
```
...
Using the Parking Violation dataset, try to create the following data frame:
...
* Construct a data frame containing Registration State and a issue date and time combined into a single DateTime column (try `paste` and `ymd_hm` from lubridate). 
    ...
    * Use these data to answer the questions:
...
```{r}
w %>% gather(day, value, d1:d31, na.rm = TRUE)
```
...
* [RStudio Data Wrangling Cheat Sheet](file:///Users/rundel/Downloads/data-wrangling-cheatsheet.pdf)
